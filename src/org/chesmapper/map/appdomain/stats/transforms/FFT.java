package org.chesmapper.map.appdomain.stats.transforms;
...
public class FFT
{
	...
	{
		return FourierTransform(2 * Math.PI, NumSamples, RealIn, ImagIn, RealOut, ImagOut);
	}
	...
	double RealIn[], double ImagIn[], double RealOut[], double ImagOut[])
	{
		...
		boolean result = FourierTransform(-2 * Math.PI, NumSamples, RealIn, ImagIn, RealOut, ImagOut);
	...
	private boolean FourierTransform(double AngleNumerator, int NumSamples, double RealIn[], double ImagIn[],
	...
	public boolean convolution(double data[], int n, double kernel[], int nk)
	{
		...
		for (int i = 0; i < n; i++)
		{
			if (((i > FreqIndex) && (i < (n2))) || ((i >= (n2)) && (i < n - FreqIndex)))
			{
				rDataOut[i] = 0;
				iDataOut[i] = 0;
			}
