package org.chesmapper.map.appdomain.stats.transforms.densityestimation;

import java.io.Serializable;

import org.chesmapper.map.appdomain.stats.transforms.FFT;
import org.chesmapper.map.appdomain.stats.transforms.Misc;
...
public class KDensity1D implements Serializable
{
	...
	public boolean estimateDensity(double a[], int n, Kernel k, int ngrid, double h, double ax, double bx,
			double weight[])
	{
		/*
		    var P, r : integer;
		 ...
		 */
		for (int i = 0; i < n; i++)
		{ // Loop over data points in x direction.
			if ((binx[i] + 1) < ngrid)
			{
