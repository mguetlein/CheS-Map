...
public class Tools
{
	...
	public static boolean ExtendedInterval(double data[], int number, double datarange[])
	{
		...
		{
			if ((r == 0) || (data[r] < datarange[0]))
				datarange[0] = data[r];
			if ((r == 0) || (data[r] > datarange[1]))
				datarange[1] = data[r];
		}
		double d = 0;
		if (number > 0)
	...
	public static boolean GaussianClusters(double datamatrix[][], int npoints, int nfactors, double[] meanvector,
			int nclusters)
	{
		/*
		  var i,j,m,k : integer;
